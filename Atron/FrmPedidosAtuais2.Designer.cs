namespace Atron
{
    partial class FrmPedidosAtuais2
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(FrmPedidosAtuais2));
            lbl_QtdeRPop = new Label();
            lblInfStatus = new Label();
            lbl_NomeProdutoRPop = new Label();
            lbl_MarcaRpop = new Label();
            lbl_CodProdutoRPop = new Label();
            pic_ProdutoPop = new PictureBox();
            lbl_CodProdutoTPop = new Label();
            lbl_NomeProdutoTpop = new Label();
            lbl_Status = new Label();
            lbl_QtdeTpop = new Label();
            lbl_MarcaPop = new Label();
            panel1 = new Panel();
            lbl_PopUpNomeProduto = new Label();
            lblInfNumPedido = new Label();
            lblNumPedido = new Label();
            lbl_FornecedorRPop = new Label();
            lbl_FornecedorPop = new Label();
            lblDataSolicitacao = new Label();
            lblInfData = new Label();
            btnOk = new Button();
            ((System.ComponentModel.ISupportInitialize)pic_ProdutoPop).BeginInit();
            panel1.SuspendLayout();
            SuspendLayout();
            // 
            // lbl_QtdeRPop
            // 
            lbl_QtdeRPop.AutoSize = true;
            lbl_QtdeRPop.Font = new Font("Microsoft Sans Serif", 21.75F, FontStyle.Regular, GraphicsUnit.Point, 0);
            lbl_QtdeRPop.ForeColor = Color.FromArgb(147, 147, 147);
            lbl_QtdeRPop.Location = new Point(223, 397);
            lbl_QtdeRPop.Name = "lbl_QtdeRPop";
            lbl_QtdeRPop.Size = new Size(47, 33);
            lbl_QtdeRPop.TabIndex = 31;
            lbl_QtdeRPop.Text = "10";
            // 
            // lblInfStatus
            // 
            lblInfStatus.AutoSize = true;
            lblInfStatus.Font = new Font("Microsoft Sans Serif", 21.75F, FontStyle.Regular, GraphicsUnit.Point, 0);
            lblInfStatus.ForeColor = Color.FromArgb(147, 147, 147);
            lblInfStatus.Location = new Point(627, 529);
            lblInfStatus.Name = "lblInfStatus";
            lblInfStatus.Size = new Size(120, 33);
            lblInfStatus.TabIndex = 30;
            lblInfStatus.Text = "Enviado";
            // 
            // lbl_NomeProdutoRPop
            // 
            lbl_NomeProdutoRPop.AutoSize = true;
            lbl_NomeProdutoRPop.Font = new Font("Microsoft Sans Serif", 21.75F, FontStyle.Regular, GraphicsUnit.Point, 0);
            lbl_NomeProdutoRPop.ForeColor = Color.FromArgb(147, 147, 147);
            lbl_NomeProdutoRPop.Location = new Point(324, 325);
            lbl_NomeProdutoRPop.Name = "lbl_NomeProdutoRPop";
            lbl_NomeProdutoRPop.Size = new Size(116, 33);
            lbl_NomeProdutoRPop.TabIndex = 29;
            lbl_NomeProdutoRPop.Text = "Produto";
            // 
            // lbl_MarcaRpop
            // 
            lbl_MarcaRpop.AutoSize = true;
            lbl_MarcaRpop.Font = new Font("Microsoft Sans Serif", 21.75F, FontStyle.Regular, GraphicsUnit.Point, 0);
            lbl_MarcaRpop.ForeColor = Color.FromArgb(147, 147, 147);
            lbl_MarcaRpop.Location = new Point(142, 258);
            lbl_MarcaRpop.Name = "lbl_MarcaRpop";
            lbl_MarcaRpop.Size = new Size(96, 33);
            lbl_MarcaRpop.TabIndex = 28;
            lbl_MarcaRpop.Text = "Marca";
            // 
            // lbl_CodProdutoRPop
            // 
            lbl_CodProdutoRPop.AutoSize = true;
            lbl_CodProdutoRPop.Font = new Font("Microsoft Sans Serif", 21.75F, FontStyle.Regular, GraphicsUnit.Point, 0);
            lbl_CodProdutoRPop.ForeColor = Color.FromArgb(147, 147, 147);
            lbl_CodProdutoRPop.Location = new Point(319, 197);
            lbl_CodProdutoRPop.Name = "lbl_CodProdutoRPop";
            lbl_CodProdutoRPop.Size = new Size(31, 33);
            lbl_CodProdutoRPop.TabIndex = 27;
            lbl_CodProdutoRPop.Text = "1";
            // 
            // pic_ProdutoPop
            // 
            pic_ProdutoPop.Image = (Image)resources.GetObject("pic_ProdutoPop.Image");
            pic_ProdutoPop.Location = new Point(542, 129);
            pic_ProdutoPop.Name = "pic_ProdutoPop";
            pic_ProdutoPop.Size = new Size(194, 195);
            pic_ProdutoPop.SizeMode = PictureBoxSizeMode.Zoom;
            pic_ProdutoPop.TabIndex = 26;
            pic_ProdutoPop.TabStop = false;
            // 
            // lbl_CodProdutoTPop
            // 
            lbl_CodProdutoTPop.AutoSize = true;
            lbl_CodProdutoTPop.Font = new Font("Microsoft Sans Serif", 26.25F, FontStyle.Underline);
            lbl_CodProdutoTPop.ForeColor = Color.FromArgb(255, 122, 0);
            lbl_CodProdutoTPop.Location = new Point(12, 191);
            lbl_CodProdutoTPop.Name = "lbl_CodProdutoTPop";
            lbl_CodProdutoTPop.Size = new Size(301, 39);
            lbl_CodProdutoTPop.TabIndex = 25;
            lbl_CodProdutoTPop.Text = "Código do Poduto:";
            // 
            // lbl_NomeProdutoTpop
            // 
            lbl_NomeProdutoTpop.AutoSize = true;
            lbl_NomeProdutoTpop.Font = new Font("Microsoft Sans Serif", 26.25F, FontStyle.Underline);
            lbl_NomeProdutoTpop.ForeColor = Color.FromArgb(255, 122, 0);
            lbl_NomeProdutoTpop.Location = new Point(10, 319);
            lbl_NomeProdutoTpop.Name = "lbl_NomeProdutoTpop";
            lbl_NomeProdutoTpop.Size = new Size(308, 39);
            lbl_NomeProdutoTpop.TabIndex = 24;
            lbl_NomeProdutoTpop.Text = "Produto Solicitado:";
            // 
            // lbl_Status
            // 
            lbl_Status.AutoSize = true;
            lbl_Status.Font = new Font("Microsoft Sans Serif", 26.25F, FontStyle.Underline);
            lbl_Status.ForeColor = Color.FromArgb(255, 122, 0);
            lbl_Status.Location = new Point(12, 523);
            lbl_Status.Name = "lbl_Status";
            lbl_Status.Size = new Size(609, 39);
            lbl_Status.TabIndex = 23;
            lbl_Status.Text = "Status (Pendente, Enviado, Entregue):";
            // 
            // lbl_QtdeTpop
            // 
            lbl_QtdeTpop.AutoSize = true;
            lbl_QtdeTpop.Font = new Font("Microsoft Sans Serif", 26.25F, FontStyle.Underline);
            lbl_QtdeTpop.ForeColor = Color.FromArgb(255, 122, 0);
            lbl_QtdeTpop.Location = new Point(12, 392);
            lbl_QtdeTpop.Name = "lbl_QtdeTpop";
            lbl_QtdeTpop.Size = new Size(205, 39);
            lbl_QtdeTpop.TabIndex = 22;
            lbl_QtdeTpop.Text = "Quantidade:";
            // 
            // lbl_MarcaPop
            // 
            lbl_MarcaPop.AutoSize = true;
            lbl_MarcaPop.Font = new Font("Microsoft Sans Serif", 26.25F, FontStyle.Underline);
            lbl_MarcaPop.ForeColor = Color.FromArgb(255, 122, 0);
            lbl_MarcaPop.Location = new Point(12, 252);
            lbl_MarcaPop.Name = "lbl_MarcaPop";
            lbl_MarcaPop.Size = new Size(124, 39);
            lbl_MarcaPop.TabIndex = 21;
            lbl_MarcaPop.Text = "Marca:";
            // 
            // panel1
            // 
            panel1.BackColor = Color.FromArgb(119, 162, 255);
            panel1.Controls.Add(lbl_PopUpNomeProduto);
            panel1.Dock = DockStyle.Top;
            panel1.Location = new Point(0, 0);
            panel1.Name = "panel1";
            panel1.Size = new Size(868, 97);
            panel1.TabIndex = 20;
            // 
            // lbl_PopUpNomeProduto
            // 
            lbl_PopUpNomeProduto.AutoSize = true;
            lbl_PopUpNomeProduto.Font = new Font("Microsoft Sans Serif", 33.75F, FontStyle.Regular, GraphicsUnit.Point, 0);
            lbl_PopUpNomeProduto.ForeColor = Color.White;
            lbl_PopUpNomeProduto.Location = new Point(350, 24);
            lbl_PopUpNomeProduto.Name = "lbl_PopUpNomeProduto";
            lbl_PopUpNomeProduto.Size = new Size(180, 52);
            lbl_PopUpNomeProduto.TabIndex = 0;
            lbl_PopUpNomeProduto.Text = "Produto";
            // 
            // lblInfNumPedido
            // 
            lblInfNumPedido.AutoSize = true;
            lblInfNumPedido.Font = new Font("Microsoft Sans Serif", 21.75F, FontStyle.Regular, GraphicsUnit.Point, 0);
            lblInfNumPedido.ForeColor = Color.FromArgb(147, 147, 147);
            lblInfNumPedido.Location = new Point(319, 135);
            lblInfNumPedido.Name = "lblInfNumPedido";
            lblInfNumPedido.Size = new Size(31, 33);
            lblInfNumPedido.TabIndex = 33;
            lblInfNumPedido.Text = "1";
            // 
            // lblNumPedido
            // 
            lblNumPedido.AutoSize = true;
            lblNumPedido.Font = new Font("Microsoft Sans Serif", 26.25F, FontStyle.Underline);
            lblNumPedido.ForeColor = Color.FromArgb(255, 122, 0);
            lblNumPedido.Location = new Point(12, 129);
            lblNumPedido.Name = "lblNumPedido";
            lblNumPedido.Size = new Size(314, 39);
            lblNumPedido.TabIndex = 32;
            lblNumPedido.Text = "Número do Pedido:";
            // 
            // lbl_FornecedorRPop
            // 
            lbl_FornecedorRPop.AutoSize = true;
            lbl_FornecedorRPop.Font = new Font("Microsoft Sans Serif", 21.75F, FontStyle.Regular, GraphicsUnit.Point, 0);
            lbl_FornecedorRPop.ForeColor = Color.FromArgb(147, 147, 147);
            lbl_FornecedorRPop.Location = new Point(213, 463);
            lbl_FornecedorRPop.Name = "lbl_FornecedorRPop";
            lbl_FornecedorRPop.Size = new Size(164, 33);
            lbl_FornecedorRPop.TabIndex = 35;
            lbl_FornecedorRPop.Text = "Fornecedor";
            // 
            // lbl_FornecedorPop
            // 
            lbl_FornecedorPop.AutoSize = true;
            lbl_FornecedorPop.Font = new Font("Microsoft Sans Serif", 26.25F, FontStyle.Underline);
            lbl_FornecedorPop.ForeColor = Color.FromArgb(255, 122, 0);
            lbl_FornecedorPop.Location = new Point(12, 457);
            lbl_FornecedorPop.Name = "lbl_FornecedorPop";
            lbl_FornecedorPop.Size = new Size(204, 39);
            lbl_FornecedorPop.TabIndex = 34;
            lbl_FornecedorPop.Text = "Fornecedor:";
            // 
            // lblDataSolicitacao
            // 
            lblDataSolicitacao.AutoSize = true;
            lblDataSolicitacao.Font = new Font("Microsoft Sans Serif", 26.25F, FontStyle.Underline);
            lblDataSolicitacao.ForeColor = Color.FromArgb(255, 122, 0);
            lblDataSolicitacao.Location = new Point(12, 607);
            lblDataSolicitacao.Name = "lblDataSolicitacao";
            lblDataSolicitacao.Size = new Size(329, 39);
            lblDataSolicitacao.TabIndex = 36;
            lblDataSolicitacao.Text = "Data da solicitação: ";
            // 
            // lblInfData
            // 
            lblInfData.AutoSize = true;
            lblInfData.Font = new Font("Microsoft Sans Serif", 21.75F, FontStyle.Regular, GraphicsUnit.Point, 0);
            lblInfData.ForeColor = Color.FromArgb(147, 147, 147);
            lblInfData.Location = new Point(347, 613);
            lblInfData.Name = "lblInfData";
            lblInfData.Size = new Size(127, 33);
            lblInfData.TabIndex = 37;
            lblInfData.Text = "25/03/25";
            // 
            // btnOk
            // 
            btnOk.Image = (Image)resources.GetObject("btnOk.Image");
            btnOk.Location = new Point(762, 598);
            btnOk.Name = "btnOk";
            btnOk.Size = new Size(106, 108);
            btnOk.TabIndex = 38;
            btnOk.UseVisualStyleBackColor = true;
            btnOk.Click += btnOk_Click;
            // 
            // FrmPedidosAtuais2
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(868, 706);
            Controls.Add(btnOk);
            Controls.Add(lblInfData);
            Controls.Add(lblDataSolicitacao);
            Controls.Add(lbl_FornecedorRPop);
            Controls.Add(lbl_FornecedorPop);
            Controls.Add(lblInfNumPedido);
            Controls.Add(lblNumPedido);
            Controls.Add(lbl_QtdeRPop);
            Controls.Add(lblInfStatus);
            Controls.Add(lbl_NomeProdutoRPop);
            Controls.Add(lbl_MarcaRpop);
            Controls.Add(lbl_CodProdutoRPop);
            Controls.Add(pic_ProdutoPop);
            Controls.Add(lbl_CodProdutoTPop);
            Controls.Add(lbl_NomeProdutoTpop);
            Controls.Add(lbl_Status);
            Controls.Add(lbl_QtdeTpop);
            Controls.Add(lbl_MarcaPop);
            Controls.Add(panel1);
            Name = "FrmPedidosAtuais2";
            Text = "FrmPedidosAtuais2";
            ((System.ComponentModel.ISupportInitialize)pic_ProdutoPop).EndInit();
            panel1.ResumeLayout(false);
            panel1.PerformLayout();
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private Label lbl_QtdeRPop;
        private Label lblInfStatus;
        private Label lbl_NomeProdutoRPop;
        private Label lbl_MarcaRpop;
        private Label lbl_CodProdutoRPop;
        private PictureBox pic_ProdutoPop;
        private Label lbl_CodProdutoTPop;
        private Label lbl_NomeProdutoTpop;
        private Label lbl_Status;
        private Label lbl_QtdeTpop;
        private Label lbl_MarcaPop;
        private Panel panel1;
        private Label lbl_PopUpNomeProduto;
        private Label lblInfNumPedido;
        private Label lblNumPedido;
        private Label lbl_FornecedorRPop;
        private Label lbl_FornecedorPop;
        private Label lblDataSolicitacao;
        private Label lblInfData;
        private Button btnOk;
    }
}